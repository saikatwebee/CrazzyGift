...
$(document).ready(() => {
    $('input[name="location"]').on("keyup keypress", (event) => {
        ...
        $("#add_cart_btn").css('background', '#5f7990');
...
$(document).on("click", "#add_cart", (event) => {
    // $("#cart_form").submit();
    var html = '<i class="fa fa-spinner fa-spin" ></i>';
    var resetbtn = "<button>Add to cart</button>";
        ...
        fetch(formAction, {
            ...
            .then(result => {
                ...
                if (result.code == 200) {
                    ...
                    // If no cart data exists in localStorage, initialize it as an empty array
                    if (!existingCartData) {
                        existingCartData = [];
                    } else {
                    ...
                    if (existingCartData.length > 0) {
                        //update if product_id exist otherwise add
...
$(document).ready(() => {
    ...
    if (carts && carts.length > 0) {
        ...
        carts.forEach((cart, key) => {
            ...
            if (cart.hasOwnProperty('custom_text') && cart.hasOwnProperty('custom_image')) {
                //if both are  exist
                html1 += '<div class="orderSummaryCard"><div class="row"><div class="col-lg-5"><div class="row"><div class="col-lg-6"><div class="imgCartPro"><img src="' + imgUrl + '" alt="cart_1"></div></div><div class="col-lg-6"><div class="nameAndPrice1"><p>' + cart.title + '</p><h3><i class="fa-solid fa-indian-rupee-sign"></i> ' + cart.price + '</h3></div><div class="quantity-selector1"><div class="quantityBtn1"><b>Quantity</b></div><button class="quantity-btn1 quantity-decrease1" data-id="' + cart.id + '">-</button><input type="text" class="quantity-input1"value="' + cart.quantity + '" data-id="' + cart.id + '" price="' + cart.price + '"><button class="quantity-btn1 quantity-increase1" data-id="' + cart.id + '">+</button></div></div></div></div><div class="col-lg-3"><div class="titleDel"><p>Delivery time</p><h6> Metros - 2 to 3 days, Tier 2 cities - 3 to 5 days, Others - 4 to 6 days</h6></div></div><div class="col-lg-4"><div class="totalPriceAM"><p>Total</p><h6><strong id="totalPriceAM_' + cart.id + '"><i class="fa-solid fa-indian-rupee-sign"></i> ' + totalPriceIndividual + '</strong></h6><div class="uploadAndDelBtn mt-4" ><button class="showimageBTN d-flex my-2" data-id="' + cart.custom_image + '" ><div class="iconshowUpload showimageBTN_div" data-id="' + cart.custom_image + '" style="margin-right: 8px;" ><i class="fa-solid fa-cloud-arrow-up showimageBTN_i" data-id="' + cart.custom_image + '" ></i> </div><span id="showimageBTN_span" data-id="' + cart.custom_image + '" >Show Image</span><i class="fa-solid fa-square-xmark" data-id="' + cart.id + '" title="Change Image" data-bs-toggle="tooltip" data-bs-placement="top" ></i></button><button class="showtextBTN d-flex my-2" data-id="' + cart.custom_text + '"><div class="iconshowUpload showtextBTN_div" style="margin-right: 8px;" data-id="' + cart.custom_text + '"><i class="fa-solid fa-envelope-open-text showtextBTN_i" data-id="' + cart.custom_text + '"></i></div><span id="showtextBTN_span" data-id="' + cart.custom_text + '">Show Message</span></button><button type="button" class="deleteBTN d-flex my-2" data-id="' + cart.id + '"><div class="iconUpload" style="margin-right: 8px;"><i class="fa-solid fa-trash-can"></i></div><span>Delete</span></button></div></div></div></div></div>';
            }
                                        ...
                                        fetch(" http://127.0.0.1:8000/guestCartImgDelete", {
                                            ...
                                            .then(data => {
                                                ...
                                                if (parsedData.length > 0) {
                                                    ...
                                                    toastr.error("Cart Details Deleted Successfully");
        ...
        .then(result => {
            ...
            if (result.code == 200) {
                ...
                if (hasBothCustomData) {
                    ...
                    console.log("Please ensure that each item is fully customized with both 'custom-image' and 'custom-message' before proceeding to the payment page.");
                   
                    toastr.error("Please ensure that each item is fully customized with both 'custom-image' and 'custom-message' before proceeding to the payment page.");     
                }
...
$(document).ready(function () {
    ...
    $(".showtextBTN,.showtextBTN_div,#showtextBTN_span,.showtextBTN_i").on("click", function (event) {
...
$(document).ready(function () {
    ...
    $(document).ready(() => {
        ...
        delAddress.forEach(function (del) {
    ...
    $(document).ready(function () {
        $('#pincode_billing').on('input', function () {
        ...
        $('#phone_billing').on('input', function () {
            var inputValue = $(this).val();
        ...
        $('#alternate_phone_billing').on('input', function () {
            var inputValue = $(this).val();
            var numericValue = getNumericTen(inputValue);
