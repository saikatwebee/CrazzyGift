        ...
        <div class="ProductInfo">
            ...
            <div class="sortOption d-flex">
                <select id="sort-select" class="sort-select">
                    <option value="{{ url('/') . '/' . $url }}">Sort by</option>
                    <option value="{{ url('/') . '/' . $url . '?query=price_high_to_low' }}"
                        {{ $query == 'price_high_to_low' ? 'selected' : '' }}> High to Low</option>
                    <option value="{{ url('/') . '/' . $url . '?query=price_low_to_high' }}"
                        {{ $query == 'price_low_to_high' ? 'selected' : '' }}>Price Low to High</option>
                    <option value="{{ url('/') . '/' . $url . '?query=latest_product' }}"
                        ...
                        <div class="products"
                            ...
                            <div class="slide-image"
                                ...
                                <button type="button" class="btn btn-sm quick-view"
                                    onclick="window.location.href='{{ url('/productDetails') . '/' . $product->slug }}';">Quick
                                    View</button>
                            </div>
                ...
                @if ($totalRecords > $recordsPerPage)
                    <div class="pagination">
                        ...
                        @for ($page = 1; $page <= ceil($totalRecords / $recordsPerPage); $page++)
                            <a href="{{ url('/') . '/' . $url . '?page=' . $page . ($query != '' ? '&query=' . $query : '') }}"
