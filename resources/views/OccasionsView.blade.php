        ...
        <div class="breadcrumb">
            <div aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Occasions</li>
                </ol>
            </div>
        </div>
        ...
        <div class="ProductInfo">
            ...
            <div class="sortOption d-flex">
                <select id="sort-select" class="sort-select">
                    <option value="{{ url('occasions') }} ">Sort by</option>
                    <option value="{{ url('occasions?query=price_high_to_low') }}"
                        {{ $query == 'price_high_to_low' ? 'selected' : '' }}> High to Low</option>
                    <option value="{{ url('occasions?query=price_low_to_high') }}"
                        {{ $query == 'price_low_to_high' ? 'selected' : '' }}>Price Low to High</option>
                    <option value="{{ url('occasions?query=latest_product') }}"
        ...
        <div class="row my-3">
            ...
            @if ($totalRecords > $recordsPerPage)
                <div class="pagination ">
                    @if ($currentPage > 1)
                        <a href="{{ $query != '' ? route('occasions', ['page' => $currentPage - 1, 'query' => $query]) : route('occasions', ['page' => $currentPage - 1]) }}"
                    ...
                    @for ($page = 1; $page <= ceil($totalRecords / $recordsPerPage); $page++)
                        <a href="{{ $query != '' ? route('occasions', ['page' => $page, 'query' => $query]) : route('occasions', ['page' => $page]) }}"
