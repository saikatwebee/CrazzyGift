        ...
        <div class="ProductInfo">
            ...
            <div class="sortOption d-flex">
                <select id="sort-select" class="sort-select">
                    <option value="{{ url('price/0-500') }} ">Sort by</option>
                    <option value="{{ url('price/0-500?query=price_high_to_low') }}"
                        {{ $query == 'price_high_to_low' ? 'selected' : '' }}> High to Low</option>
                    <option value="{{ url('price/0-500?query=price_low_to_high') }}"
                        {{ $query == 'price_low_to_high' ? 'selected' : '' }}>Price Low to High</option>
                    <option value="{{ url('price/0-500?query=latest_product') }}"
        ...
        <div class="row my-3">
            ...
            @if ($totalRecords > $recordsPerPage)
                <div class="pagination ">
                    @if ($currentPage > 1)
                        <a href="{{ $query != '' ? route('product-price-low', ['page' => $currentPage - 1, 'query' => $query]) : route('product-price-low', ['page' => $currentPage - 1]) }}"
                    ...
                    @for ($page = 1; $page <= ceil($totalRecords / $recordsPerPage); $page++)
                        <a href="{{ $query != '' ? route('product-price-low', ['page' => $page, 'query' => $query]) : route('product-price-low', ['page' => $page]) }}"
