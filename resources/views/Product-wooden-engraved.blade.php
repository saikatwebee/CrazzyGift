    ...
    <section class="loginRegister container">
        ...
        <div class="ProductInfo">
            ...
            <div class="sortOption d-flex">
                ...
                <select id="sort-select" class="sort-select">
                    ...
                    <option value="{{url('products/wooden-engraved')}}">Sort by</option>
                    <option value="{{ url('products/wooden-engraved?query=price_high_to_low') }}" {{($query=="price_high_to_low") ? "selected" : ""}}> High to Low</option>
                    <option value="{{ url('products/wooden-engraved?query=price_low_to_high') }}" {{($query=="price_low_to_high") ? "selected" : ""}}>Price Low to High</option>
                    <option value="{{ url('products/wooden-engraved?query=latest_product') }}" {{($query=="latest_product") ? "selected" : ""}}>Latest Collection</option>
        ...
        <div class="row my-3">
            ...
            <div class="pagination ">
                @if ($currentPage > 1)
                    <a href="{{ ($query!="") ? route('wooden-engraved', ['page' => $currentPage -1, 'query' =>  $query]) : route('wooden-engraved', ['page' => $currentPage - 1]) }}"
                ...
                @for ($page = 1; $page <= ceil($totalRecords / $recordsPerPage); $page++)
                    <a href="{{ ($query!="") ? route('wooden-engraved', ['page' => $page, 'query' =>  $query]) : route('wooden-engraved', ['page' => $page]) }}"
