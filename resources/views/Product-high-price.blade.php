        ...
        <div class="ProductInfo">
            ...
            <div class="sortOption d-flex">
                <select id="sort-select" class="sort-select">
                    <option value="{{ url('price/2000-above') }} ">Sort by</option>
                    <option value="{{ url('price/2000-above?query=price_high_to_low') }}"
                        {{ $query == 'price_high_to_low' ? 'selected' : '' }}> High to Low</option>
                    <option value="{{ url('price/2000-above?query=price_low_to_high') }}"
                        {{ $query == 'price_low_to_high' ? 'selected' : '' }}>Price Low to High</option>
                    <option value="{{ url('price/2000-above?query=latest_product') }}"
        ...
        <div class="row my-3">
            ...
            @endforeach
                ...
                <div class="pagination ">
                    @if ($currentPage > 1)
                        <a href="{{ $query != '' ? route('product-price-high', ['page' => $currentPage - 1, 'query' => $query]) : route('product-price-high', ['page' => $currentPage - 1]) }}"
                    ...
                    @for ($page = 1; $page <= ceil($totalRecords / $recordsPerPage); $page++)
                        <a href="{{ $query != '' ? route('product-price-high', ['page' => $page, 'query' => $query]) : route('product-price-high', ['page' => $page]) }}"
