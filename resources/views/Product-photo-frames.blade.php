        ...
        <div class="ProductInfo">
            ...
            <div class="sortOption d-flex">
                <select id="sort-select" class="sort-select">
                    ...
                    <option value="{{ url('products/photo-frames') }}">Sort by</option>
                    <option value="{{ url('products/photo-frames?query=price_high_to_low') }}" {{($query=="price_high_to_low") ? "selected" : ""}}> High to Low</option>
                    <option value="{{ url('products/photo-frames?query=price_low_to_high') }}" {{($query=="price_low_to_high") ? "selected" : ""}}>Price Low to High</option>
                    <option value="{{ url('products/photo-frames?query=latest_product') }}" {{($query=="latest_product") ? "selected" : ""}}>Latest Collection</option>
        ...
        <div class="row my-3">
            ...
            @endforeach
               ...
               <div class="pagination ">
                   @if ($currentPage > 1)
                       <a href="{{ ($query!="") ? route('photo-frames', ['page' => $currentPage -1, 'query' =>  $query]) : route('photo-frames', ['page' => $currentPage - 1]) }}"
                   ...
                   @for ($page = 1; $page <= ceil($totalRecords / $recordsPerPage); $page++)
                       <a href="{{ ($query!="") ? route('photo-frames', ['page' => $page, 'query' =>  $query]) : route('photo-frames', ['page' => $page]) }}"
